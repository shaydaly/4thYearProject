...
def handler(event, context):
    ...
    query = """select {0}, {1},{2},{3},{4},{5},{7} from journey,customer where journey.userid = customer.userid AND (customer).username='{6}' AND (journey).endLatitude != 0 order by journeyid desc""".format(startLatitude,startLongitude,endLatitude,endLongitude,startTime,endTime, username,journeyID)
    try:
